...
#' @param system_message A specific instruction given to the API to ensure it understands the context and responds accordingly. The default message is "You are an expert sentiment analyzer, analyzing emotion from text. Please carefully read the following text and respond with '-1' if the sentiment is negative, '0' if it's neutral, or '1' if it's positive."
#' @param temperature Controls randomness in the API's responses. Lower value makes the output more deterministic. The default value is 0.2.
#' 
#' @details 
#' \itemize{
...
#' }
#' 
#' @return Returns a dataframe containing the original ID, Text, the extracted sentiment and a flag indicating whether the sentiment extraction was successful or not.
#'
#' @examples
#' # Load required libraries
#' library(httr)
...
#' 
#' # Use the function
#' output <- gpt_sentiment(df, system_message = "You are an expert sentiment analyzer, analyzing emotion from text. Please carefully read the following text and respond with '-1' if the sentiment is negative, '0' if it's neutral, or '1' if it's positive.", temperature = 0.2)
#' 
#' # View the output
#' print(output)
#' 
